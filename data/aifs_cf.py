...
parser = argparse.ArgumentParser(description="Convert Aifs output to CF-compliant NetCDF")
...
files = sorted(file_path.glob("prediction_date*.nc"))
#file = files[0]
...
for idx, file in enumerate(files):
    ...
    source = Path("/discover/nobackup/projects/QEFM/data/FMAifs/nc_files")
    tmp_file = source / f"aifs-dataset-source-era5_date-{yyyy}-{mm}-{dd}_res-0.25_levels-13_steps-0.nc"
    ds_temp = xr.open_dataset(tmp_file)
    pv = ds_temp['geopotential_at_surface'].isel(time=0).squeeze().to_numpy()
    ds['PHIS'] = (['lat', 'lon'], pv)
