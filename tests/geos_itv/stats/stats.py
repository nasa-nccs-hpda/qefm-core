...
ALL_coll_nms = ['default', 'slices']

VARS_LONG_MAP = {'H': 'Heights', 'U': 'U-Wind', 'V': 'V-Wind',
                 'T': 'Temperature', 'Q': 'Specific Humidity',
                 'P': 'Sea-Level Pressure', 'Q2M': '2m Specific Humidity',
                 'T2M': '2m Temperature', 'U10M': '10m U-Wind',
                 'V10M': '10m V-Wind', 'D2M': '2m Dew Point'}

VARS_UNIT_MAP = {'H': 'm', 'U': 'm/s', 'V': 'm/s', 'T': 'K', 'Q': 'g/kg',
                 'P': 'hPa', 'Q2M': 'g/kg', 'T2M': 'K', 'U10M': 'm/s',
                 'V10M': 'm/s', 'D2M': 'K'}

# Define exceptions for units
SQUARED_UNIT_STATS = ['mse', 'rms_dis', 'rms_dsp']
UNITLESS_STATS     = ['acorr']

VARS_SCALE_MAP = {'H': 1., 'U': 1., 'V': 1., 'T': 1., 'Q': 1000., 'P': 1./100.,
                  'Q2M': 1000., 'T2M': 1., 'U10M': 1., 'V10M': 1., 'D2M': 1.}
...
class BatchDatasetProcessor:
    ...
    def from_yaml(cls, yaml_file):
        ...
        config = cls._convert_yaml_config(yaml_config)
        return cls(config=config)

    @staticmethod
    def _convert_yaml_config(yaml_config):
    ...
    def _generate_output_filenm(self, dataset_type: str, 
        ...
        # Standard detailed name
            ...
            filenm = (f'{prefix}_{coll_part}{model_nm}_{date_range}_'
                        ...
                        f'spc{fcst_spacing}d_{grid_size}.nc4')
        return filenm
        ...
        for dataset_type, (filenm, model_nm) in expected_files.items():
            ...
            for search_dir in search_dirs:
                ...
                if os.path.exists(full_path):
                    found_datasets[dataset_type] = full_path
                    ...
                    try:
                        ...
                        try:
                            if shift_str.endswith('h'):
                                shift_hours = int(shift_str[:-1])
                            elif shift_str.endswith('d'):
                                shift_hours = int(shift_str[:-1]) * 24
                            else:
                                shift_hours = int(shift_str)  # Assume hours
                        except ValueError:
    ...
    def _validate_dataset(self, dataset_type, all_files, no_template_colls, 
        ...
        try:
            ...
            if collections_needing_fallback and 'default' in sample_files:
                            ...
                            with xr.open_dataset(file_path, 
                                ...
                                if 'time' in ds.coords:
                                    ...
                                    for required_lead in leads:
                                        if required_lead not in available_leads:
    ...
    def _calculate_monthly_weights(self, dt: datetime) -> tuple:
        ...
        # Calculate distance from start and end of month
        dist_from_start = day - 1  # 0-based
        dist_from_end = days_in_month - day

        # Determine which two months to blend
        if dist_from_start < dist_from_end:
            # Closer to start of month: blend with previous month
            prev_month = month - 1 if month > 1 else 12
            prev_year = year if month > 1 else year - 1
            # Weight based on distance from start
            weight_prev = (days_in_month - day) / days_in_month
            weight_curr = day / days_in_month
            ...
            # Weight based on distance from end
            weight_curr = (days_in_month - day + 1) / days_in_month
            weight_next = (day - 1) / days_in_month
    ...
    def _regrid_multi_lead_datasets(self, multi_lead_datasets, needed_fhours, 
        ...
        Returns:
        - regridded_cache: Dict {coll_nm: {fhour: {var_nms: dataarray}}}
        '''
        regridded_cache = {}
            ...
            for fhour in needed_fhours:
                ...
                try:
                    regridded_vars, _ = self._process_dataset_variables(
                        fhour_ds, [coll_nm], file_type, validation_result)
                except Exception as e:
                    raise ValueError(f'[Failed to regrid variables for '
                                     f'{coll_nm} collection at fhour='
                                     f'{fhour}: {str(e)}')
    ...
    def _load_and_process_files_for_init_date(self, files_by_key, file_type, 
                                              coll_modes, validation_result):
        ...
        # Separate multi-lead and single-lead collections
        ...
        for i, key in enumerate(sorted_keys):
            ...
            if fhour == sorted_keys[0][1]:  
                ...
                # Get multi-lead data for this fhour
            ...
            # Subsequent fhours: use cached multi-lead data
            else:
                # Process single-lead files only
                single_files = {name: path for name, path in 
                                coll_files.items() if name 
                                in single_lead_colls}
                single_result = None
                if single_files:
                    single_result = self._load_and_process_files(
                        single_files, file_type, validation_result)
    ...
    def _combine_into_structured_datasets(self, loaded_data, init_dates, leads,
                                          val_results=None, 
        ...
        if 'clim' in loaded_data and loaded_data['clim']:
            ...
            print(f'      {start_time.strftime("%b %d")} = {first_weight1:.1%}'
                  f'month {first_month1} + {first_weight2:.1%} month '
                  f'{first_month2}')
            print(f'      {end_time.strftime("%b %d")} = {last_weight1:.1%}'
                  f'month {last_month1} + {last_weight2:.1%} month '
                  f'{last_month2}')
            ...
            for time_val in unique_valid_times:
                ...
                if cycle in loaded_data['clim']:
                    ...
                    # Apply monthly interpolation (weighted average_)
                    ...
                    print(f'    Warning: No climatology data for cycle '
                          f'{cycle}z at time {time_val}')
            # Combine climatology
        ...
        # Add grid weights to all datasets
    ...
    def process_batch(self, target_coll=None, info_dir=None, 
        ...
        # Parse date range from config accounting for spacing
        spacing = self.config.get('fcst_spacing', 1)
        init_dates = self._parse_date_range_with_spacing(
            self.config['FDATES'], spacing)
        leads = self._generate_leads(
            self.config['FDAYS'], self.config['NFREQ'])
    
        # Filter dates by indices if specified
        if date_start_idx is not None or date_end_idx is not None:
            # Get valid indices
            start_idx = 0 if date_start_idx is None else max(0, min(
                date_start_idx, len(init_dates)-1))
            end_idx = len(init_dates)-1 if date_end_idx is None else max(
                0, min(date_end_idx, len(init_dates)-1))
            # Filter init_dates
            if start_idx <= end_idx:
                filtered_init_dates = init_dates[start_idx:end_idx+1]
                print(f'[INFO] Processing date subset: indices {start_idx}-'
                      f'{end_idx}, {len(filtered_init_dates)} dates')
                init_dates = filtered_init_dates
            else:
        ...
        # Calculate unique valid times for analysis files
        ...
        combined_datasets = self._combine_into_structured_datasets(
            loaded_data, init_dates, leads, val_results, skip_calc_mode
        )
    ...
    def merge_collection_datasets(self, dataset_type: str, info_dir: str
        ...
        try:
            ...
            # Step 4: Filter to final variables and add scaling
            ...
            # Step 6: Delete collection files after successful merge
...
class StatisticsProcessor:
    ...
    def __init__(self, config: Dict[str, Any], dataset_files: Dict[str, str], 
                 init_dates: List[datetime], leads: List[int]):
    ...
    def _load_and_validate_datasets(self):
        ...
        for dataset_type, file_path in self.dataset_files.items():
            if not os.path.exists(file_path):
                raise ValueError(f'{dataset_type} file not found: {file_path}')
        ...
        for mask, weight_type, dims_info in [
            ...
            if mask is not None:
                if weight_type == 'weights_3d':
                    weights = base_weights.broadcast_like(mask)
                else:
    ...
    def _create_regional_masks(self, requested_regions):
        ...
        # region_files_map = {
        #     'CUS': ('/Users/mefrazer/Data/Gridding/CONUS_mask.nc', 'CONUS'),
        #     'LND': ('/Users/mefrazer/Data/Gridding/land_sea_mask_0.5deg.nc', 
        #             'land_mask')
        # }
                    ...
                    for var in self.stats_arrays.keys():
                        ...
                        if 'lev' in fcst_data[var].dims:
                            ...
                            for k in range(n_levels):
                                # Select 3d weights level slices
                                weights = self.enhanced_weights['weights_3d'
                                                                ][k, :, :]
    ...
    def _save_statistics_output(
        ...
        suffix = os.path.basename(fcst_filenm).split(f'{fcst_model}_', 1)[1]
        if stats_type == 'reg':
        ...
        if stats_type == 'reg':
            self._write_scorecard_files(suffix)

        return stats_ds
    
    def _write_scorecard_files(self, suffix):
        ...
        scorecard_dir = (f'output/scorecard_input_{model}_{ana_model}_'
        ...
        def apply_q_scaling(var, stat_key, value):
            '''Helper: revert Q vars to units of kg/kg for scorecards'''
        ...
        def process_stats_for_var(
            ...
            for stat_key, stat_nm in stat_mapping.items():
                if stat_key in self.stats_arrays[var]:
                    ...
                    value = apply_q_scaling(var, stat_key, value)
                    write_line(file, pressure, var, stat_nm, 
                               self.leads[lead_idx], value, date_str, 
                               domain_nm, north, south, east, west)
        
        # Header line
        header = ('count|date|domain_nm|east|expver|forecast|level|levtype|'
        ...
        for i, init_date in enumerate(self.init_dates):
            ...
            filenm = f'{scorecard_dir}/{model}.fstat.stds.log.{file_date}.txt'
                        ...
                        for coll in fvars.keys():
                            for var in fvars[coll]:
                                if var in self.stats_arrays:
                                    if is_3d[coll]:
                                        # 3D variables
                                        pressure_levels = self.stats_arrays[
                                            var]['rms'].lev.values
                                        ...
                                        # 2D variables
                                        pressure = (1000.0 if coll[:2] == 'de'
                                                    else 0.0)
                                        process_stats_for_var(
                                            file, var, pressure, None, 
                                            date_str, domain_nm, north, south, 
                                            east, west, i, j, region_idx)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Forecast Processing and Statistics')
    ...
    stats_group.add_argument('--stats', help=('Run statistics calculation '
                                              'type: reg or glo'))
    ...
    stats_group.add_argument('--chunk_size', type=int, default=3, 
                             help='Forecast chunk size (default: 3 init_date)')

    # Merge options
    merge_group = parser.add_argument_group('Merge options')
    merge_group.add_argument('--merge_collections', type=str, 
                             help=('Dataset to merge collections '
                                   '(fcst, ana, or clim)'))
    merge_group.add_argument('--clean', action='store_true', 
                             help='Merge chunked statistics files')
    merge_group.add_argument('--type', type=str, 
...
def main():
    ...
    if not (args.fcst or args.ana or args.clim or args.process or args.stats):
        args.process = True
    
    # Validation of arguments
    # Check for required info_dir
    if not args.info_dir and (args.check_only or 
                              args.init_start_idx is not None or 
                              args.init_end_idx is not None or 
                              args.clean or args.target_coll or 
                              args.date_start_idx is not None or 
                              args.date_end_idx is not None or 
                              args.merge_collections is not None):
        print('[ERROR] --info_dir is required for --check_only, chunked '
              'processing, --clean, --collection, --merge_collections, or '
              'forecast date range processing')
        sys.exit(1)
    ...
    try:
        ...
        if not (args.stats and not args.process):
        ...
        if args.stats:
            ...
            stats_processor = StatisticsProcessor(
                processor.config, dataset_files, init_dates, leads)
