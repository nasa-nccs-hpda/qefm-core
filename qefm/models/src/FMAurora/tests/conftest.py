...
def test_input_output() -> Generator[tuple[Batch, SavedBatch], None, None]:
    ...
    path = hf_hub_download(
        repo_id="microsoft/FMAurora",
        filename="FMAurora-0.25-small-pretrained-test-input.pickle",
    )
    ...
    path = hf_hub_download(
        repo_id="microsoft/FMAurora",
        filename="FMAurora-0.25-static.pickle",
    )
    with open(path, "rb") as f:
    ...
    path = hf_hub_download(
        repo_id="microsoft/FMAurora",
        filename="FMAurora-0.25-small-pretrained-test-output.pickle",
    )
    with open(path, "rb") as f:
