...
output_path = Path("/discover/nobackup/projects/QEFM/data/rollout_outputs/FMAurora/GT")
...
model.load_checkpoint_local("/discover/nobackup/projects/QEFM/qefm-core/qefm/models/checkpoints/aurora/aurora-0.25-pretrained.ckpt")
...
def to_fluid(predvar, filepath, dt):
        ...
        print(f"[]-----to fluid dt top of fxn int(Ymd): {np.int32(dt.strftime('%Y%m%d'))}")
        print(f"[]-----to fluid dt top of fxn inst(HMS): {np.int32(dt.strftime('%H%M%S'))}")
        ...
        # change time elapsed to minutes instead of days and add attributes
        dt_lst = list(predvar.metadata.time)
        time_lst = [np.datetime64(dt.strftime('%Y-%m-%d %H:%M')) for d in dt_lst]
        reference_time = np.datetime64(dt.strftime('%Y-%m-%d %H:%M'))
        time_lst = (time_lst - reference_time) / np.timedelta64(1, "h")
        time_lst = [np.float32(elem) for elem in time_lst]
        #print(f'==== time list {time_lst}')
        
        # fix longitude
        lons = _np(predvar.metadata.lon)
        nlons = len(lons)
        shiftx = int(nlons/-2)
        lons = np.roll(lons, shiftx)
        lon = ((lons + 180) % 360) - 180
        lon = [np.float32(elem) for elem in lon]
        #print(f'===== lon og {predvar.metadata.lon}')
        #print(f'===== new lon {lon}, {np.max(lon)}, {np.min(lon)}')
        
        # define lats
        lat = _np(predvar.metadata.lat)
        
        # levs
        levs = list(predvar.metadata.atmos_levels)
        levs = [np.float32(lev) for lev in levs]

        
        ds = xr.Dataset(
            {
                **{
                    surf_var_dict[k]["var_name"]: (("time", "lat", "lon"), _np(v.squeeze(dim=0)), {"units": surf_var_dict[k]["units"], "long_name": surf_var_dict[k]["long_name"], "_FillValue": fill_value, "missing_value": fill_value, "fmissing_value": fill_value})
                    for k, v in predvar.surf_vars.items()
                },
                **{
                    static_var_dict[k]["var_name"]: (("lat", "lon"), _np(v), {"units": static_var_dict[k]["units"], "long_name": static_var_dict[k]["long_name"], "_FillValue": fill_value, "missing_value": fill_value, "fmissing_value": fill_value})
                    for k, v in predvar.static_vars.items()
                },
                **{
                    atmos_var_dict[k]["var_name"]: (("time", "lev", "lat", "lon"), _np(v.squeeze(dim=0)), {"units": atmos_var_dict[k]["units"], "long_name": atmos_var_dict[k]["long_name"], "_FillValue": fill_value, "missing_value": fill_value, "fmissing_value": fill_value})
                    for k, v in predvar.atmos_vars.items()
                },
            },
            coords={
                "time": ("time", time_lst),
                "lev": ("lev", levs, {"units": "hPa", "long_name": "pressure_level", "stored_direction": "decreasing", "positive": "down"}),
                "lat": ("lat", lat),
        ...
        if lats[0] != -90:
            lats = np.insert(lats, 0, -90.)  # prepend assuming lats already in ascending order

        ds = ds.reindex(lat=lats, fill_value=fill_value)
        lats = [np.float32(elem) for elem in lats]
        ds.coords['lat'] = lats  # Now explicitly update the latitudes in the dataset
        ds['lat'].attrs = {"units": "degrees_north", "long_name": "latitude"}
        
        
        
        print(f"[]-----to fluid dt before begin_date int(Ymd): {np.int32(dt.strftime('%Y%m%d'))}")
        print(f"[]-----to fluid dt before begin_date int(HMS): {np.int32(dt.strftime('%H%M%S'))}")
        
        # Update the time attribute (metadata) 
        begin_date = np.int32(dt.strftime('%Y%m%d'))
        begin_time = np.int32(dt.strftime('%H%M%S'))
        time_incr = np.int32(60000)
        ds["time"].attrs["units"] = f"hours since {dt.strftime('%Y-%m-%d %H:%M')}"
        ds["time"].attrs["begin_date"] = begin_date
        ds["time"].attrs["begin_time"] = begin_time
        ds["time"].attrs["time_increment"] = time_incr
        ...
        g = 9.80665 
        ds["H"] = ds["Z"] / g
        ds["H"].attrs = {"units": "m", "long_name": "heights", "_FillValue": fill_value}
        
        ds.attrs = {
		"Conventions": "CF" ,
		"Comment" : "NetCDF-4" ,
		"Institution" : "NASA Global Modeling and Assimilation Office" ,
		"Title" : "Aurora_AI forecast" 
        }
    
        # convert all variables to float32
        ds = ds.map(lambda x: x.astype(np.float32))
        
        #print("==== TO FLUID ",ds)
        #encx = dict(_FillValue= fill_value)
        #enc = {var: encx for var in ds.data_vars}
        ds.to_netcdf(filepath, mode="w", format="NETCDF4") 
...
start_date = datetime(2024, 12, 31, 0, 0)
#start_date = datetime(2024, 1, 1, 0, 0)
end_date = datetime(2025, 12, 31, 0, 0)
#end_date = datetime(2025, 12, 31, 18, 0)
...
while current_date <= end_date:
    ...
    to_fluid(initial_state, write_path, current_date)   
    ...
    for i in range(len(preds)):
        ...
        pred_date = current_date + timedelta(hours=6*i)
        to_fluid(pred, write_path, pred_date)   
