...
#relative_dataset_file = "/discover/nobackup/projects/QEFM/qefm-core/data/gt-temp/gencast-dataset-source-era5_date-2024-12-01_res-0.25_levels-13_steps-20.nc"
...
print('time_steps = pd.date_range(start=start_time, periods=nsteps, freq="12h")')
...
print('ds = ds.assign_coords(datetime=ds["time"]')
ds = ds.assign_coords(datetime=ds["time"])

print("drop the time dimension for land_sea_mask and geopotential_at_surface")
...
print("expand the dimensions")
for var in var_2d + var_3d + ['datetime']:
    ds[var] = ds[var].expand_dims("batch")

print('ds = ds.rename({"total_precipitation": "total_precipitation_6hr",})')
ds = ds.rename({"total_precipitation": "total_precipitation_6hr",})

print('example_batch = ds.compute()')
example_batch = ds.compute()
...
#    example_batch, target_lead_times=slice("12h", f"{train_steps*12}h"),
    **dataclasses.asdict(task_config))

eval_inputs, eval_targets, eval_forcings = data_utils.extract_inputs_targets_forcings(
    example_batch, target_lead_times=slice("6h", f"{eval_steps*6}h"),
#    example_batch, target_lead_times=slice("12h", f"{eval_steps*12}h"),
    **dataclasses.asdict(task_config))
