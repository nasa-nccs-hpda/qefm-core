...
print("##Compute Graphcast chunked prediction from subsetted ERA5 data:") 
...
print("[XLA_PYTHON_CLIENT_ALLOCATOR] = ", os.environ["XLA_PYTHON_CLIENT_ALLOCATOR"])
...
parser.add_argument("--tsteps", "-t", default="42", type=str, help="# Training steps")
parser.add_argument("--esteps", "-e", default="42", type=str, help="# Eval steps")
...
parser.add_argument("--pythonpath", "-p", default="None", type=str, help="Python path")
...
var=f"{args.var}"
pythonpath=f"{args.pythonpath}"
print("arguments:", args._get_kwargs)

ppath=str(os.environ.get('PYTHONPATH', '').split(os.pathsep))
...
if os.path.exists(Path(predfile)):
  ...
  #absolute_path = Path('/explore/nobackup/projects/ilab/projects/QEFM/qefm-core/qefm/models/checkpoints/graphcast/params_GraphCast_3d.npz')
...
#/explore/nobackup/projects/ilab/projects/QEFM/data/FMGraphCast/6h/Y2024/graphcast-dataset-source-era5_date-2024-12-01_res-0.25_levels-3_freq-6h_steps-1.nc 
  dataset_file=Path(infile)
  print("dataset_file:\n", dataset_file, "\n")
  with open(dataset_file, "rb") as f:
      #example_batch = xarray.load_dataset(f).compute()
      example_batch = xarray.load_dataset(f)

  assert example_batch.dims["time"] >= 3  # 2 for input, >=1 for targets

  train_steps = int(args.tsteps)
  eval_steps = int(args.esteps)
  print("example_batch.dims[time]: ", str(example_batch.dims["time"]))
  ...
  # print('target_lead_times=slice("6h", f"{eval_steps*6}h"')
  # train_inputs, train_targets, train_forcings = data_utils.extract_inputs_targets_forcings(
  #     example_batch, target_lead_times=slice("6h", f"{train_steps*6}h"),
  #     **dataclasses.asdict(task_config))
  #print("\ntrain_inputs, train_targets, train_forcings: " , train_inputs, train_targets, train_forcings ) 

  if var=='All':
    targets_list = task_config.target_variables
  else:
    targets_list = [var]
  tuple_targets_from_list = tuple(targets_list)
  
  # task_config2 = graphcast.TaskConfig(
  #   # input_variables=task_config.input_variables,
  #   # target_variables=task_config.target_variables,
  #   input_variables=tuple_targets_from_list,
  #   target_variables=tuple_targets_from_list,
  #   forcing_variables=task_config.forcing_variables, 
  #   pressure_levels=tuple_from_list,
  #   input_duration="6h",
  # )

  levs3_list = [1, 500, 825]
  tuple_from_lev_list = tuple(levs3_list)
  
  if levs=='3':
    task_config4 = graphcast.TaskConfig(
      input_variables=tuple_targets_from_list,
      target_variables=tuple_targets_from_list,
      forcing_variables=task_config.forcing_variables, 
      pressure_levels=tuple_from_lev_list,
      input_duration="6h",
    )
    task_config=task_config4

  print("\ntask_config: ", str(task_config))

  # eval_inputs, eval_targets, eval_forcings = data_utils.extract_inputs_targets_forcings(
  #     example_batch, target_lead_times=slice("6h", f"{eval_steps*6}h"), #pressure_levels=tuple_from_list, 
  #     **dataclasses.asdict(task_config2))

  # lead=f"{eval_steps*6}h"
  # print("target_lead_times=",lead)
  # eval_inputs, eval_targets, eval_forcings = data_utils.extract_inputs_targets_forcings(
  #     example_batch, target_lead_times=slice("6h", f"{eval_steps*6}h"), #pressure_levels=tuple_from_list, 
  #     **dataclasses.asdict(task_config))
  ...
  eval_inputs, eval_targets, eval_forcings = data_utils.extract_inputs_targets_forcings(
    ...
    **dataclasses.asdict(task_config))
  #print("\neval_inputs, eval_targets, eval_forcings: ", eval_inputs, eval_targets, eval_forcings)
  ...
  print("Eval inputs should ?? match number of time-steps: ", str(esteps))
  ...
  def drop_state(fn):
    turn_sam_turn=lambda **kw: fn(**kw)[0]
    print(f"turn_sam_turn {turn_sam_turn}")
    return turn_sam_turn
  ...
  if pythonpath != 'None':
    ...
    print(f"Number of local devices {num_ensemble_members}")
    ...
    print("# Use pmapped version to parallelise across devices. chunked_prediction_generator_multiple_runs()")
    chunks = []
    # GST
    ...
    print("# predictions = rollout.chunked_prediction")
    predictions = rollout.chunked_prediction(
        run_forward_jitted,
        rng=jax.random.PRNGKey(0),
        inputs=eval_inputs,
        targets_template=eval_targets * np.nan,
#        num_steps_per_chunk = eval_steps,
        forcings=eval_forcings)
  predictions
