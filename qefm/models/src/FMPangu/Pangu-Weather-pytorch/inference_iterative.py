...
def load_input_surface(input_data_dir: str, input_file: str, tidx:int = 0) -> np.ndarray:
    ...
    data = np.stack([ds[var].isel(valid_time=tidx).sel(levels, dims='pressure_level').values for var in variables], axis=0)
    return data
...
if __name__ == '__main__':
    ...
    init_datetime = pd.to_datetime(start_time)
    YYYY = f"{init_datetime.year:04d}"
